 ...
 * 2/23/14
 * CSC 220
 * To pass in a text file simply add it to the command line parameters.
 * Do Not redirect it. Couldn't get that part figured out.
 **/
...
void ReverseOrder(TreeNode *&root, bool b) {
    bool header = b;
    if (header) {
    ...
    if (root != NULL) {
        ReverseOrder(root->right, header);
        cout << root->c;
        ReverseOrder(root->left, header);
    }
}

void funPrint(TreeNode *&n, int lev) {
        if (n != NULL) {
            funPrint(n->right, lev + 1);
        
            funPrint(n->left, lev + 1);
            for (int i=0; i<lev; i++)
                cout << "   ";
            cout << n->c << endl;
...
void execute(char p[], int size) {
    ...
    ReverseOrder(root, true); cout << endl;
    funPrint(root, 0);
    cout << "\n\n\n";
}
...
int main(int argc, char *argv[]) {
    ...
    storeCharArrays(str,line4, 65, 82);      //storeCharArray passing in 2 or more array
    storeCharArrays(str,line5, 83, 92);      //parameters created problems.
